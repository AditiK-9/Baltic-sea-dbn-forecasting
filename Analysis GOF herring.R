...
library(readxl)
wb <- read_excel("C://Aditi//UNI//Data Science//PX5902 (2024-25) Data Science Project//DataDirFP//Gulf_of_Finland_masters_students.xlsx", sheet = 1)
head(wb)
summary(wb)
md.pattern(wb)

af <- c("SST_spr", "SST_su", "SHT_su", "SAL_win","SAL_deep", "pH", "Ox_deep", "DIP", 
        "TP", "TN", "DIN", "N_load", "P_load", "Ice_max", "WBSI")
...
str(logcc6)
multi_hmm <- depmix(list(SST_spr ~ 1, SST_su ~ 1, SHT_su ~ 1, SAL_win ~ 1,SAL_deep ~ 1, pH ~ 1, Ox_deep ~ 1, 
                         DIP ~ 1, TP ~ 1, TN ~ 1, DIN ~ 1, N_load ~ 1, P_load ~ 1, Ice_max ~ 1, WBSI ~ 1, 
                         Herr_Recruits ~ 1),
...
emission_prob <- getpars(multi_hmm_fit)[7:70]
emission_prob2 <- matrix(emission_prob, nrow = 2, byrow = TRUE)
rownames(emission_prob2) <- c("R1", "R2")
vars <- c("SST_spr", "SST_su", "SHT_su", "SAL_win","SAL_deep", "pH", "Ox_deep", "DIP", 
          "TP", "TN", "DIN", "N_load", "P_load", "Ice_max", "WBSI", "Herr_Recruits")
...
model_string <- "[State_t][State_t1|State_t][Ox_deep_t|State_t][WBSI_t|State_t][Herr_Recruits_t|State_t:Ox_deep_t:WBSI_t][Ox_deep_t1|Ox_deep_t:State_t1][WBSI_t1|WBSI_t:State_t1][Herr_Recruits_t1|Herr_Recruits_t:WBSI_t1:Ox_deep_t1]"
