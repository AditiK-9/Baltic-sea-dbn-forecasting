...
multi_hmm <- depmix(list(SST_spr ~ 1, SHT_su ~ 1, SST_su ~ 1, SAL_win ~ 1, SAL_deep ~ 1, pH ~ 1, 
                         ...
                         N_load ~ 1, P_load ~ 1, WBSI ~ 1, Herr_recruits ~ 1),
...
vars <- c("SST_spr", "SHT_su", "SST_su", "SAL_win", "SAL_deep", "pH", "Ox_deep", "DIP", "TP", "Si", "TN", 
          "DIN", "Ice_max", "N_load", "P_load", "WBSI", "Herr_recruits")
...
dbn_data <- logcc6.1[, c("State", "Ox_deep", "WBSI", "Herr_recruits")]
dbn_data
dbn_data2 <- dbn_data %>%
  mutate(across(c(State, Ox_deep, WBSI, Herr_recruits), 
                ~lag(.), .names = "{.col}_t")) %>%
  rename_with(~paste0(.x, "_t1"), c(State, Ox_deep, WBSI, Herr_recruits)) %>%
  filter(!is.na(State_t))
dbn_data2

model_string <- "[State_t][State_t1|State_t][Ox_deep_t|State_t][WBSI_t|State_t][Herr_recruits_t|State_t:Ox_deep_t:WBSI_t][Ox_deep_t1|Ox_deep_t:State_t1][WBSI_t1|WBSI_t:State_t1][Herr_recruits_t1|Herr_recruits_t:WBSI_t:Ox_deep_t1]"
...
actual_values <- dbn_data2$Herr_recruits_t
plot_df <- data.frame(
  Year = rep(years, 2),
