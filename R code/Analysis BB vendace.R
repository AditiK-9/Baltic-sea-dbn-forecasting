...
library(readxl)
wb <- read_excel("C://Aditi//UNI//Data Science//PX5902 (2024-25) Data Science Project//DataDirFP//Bothnian_Bay_masters_students.xlsx", sheet = 1)
head(wb)
summary(wb)
md.pattern(wb)

af <- c("SST_spr", "SHT_su", "SST_su", "SAL_win", "SAL_deep", "pH", "Ox_deep", "DIP", "TP", "Si", "TN", "DIN", "Ice_max", "N_load", "P_load", "WBSI")

cc1 <- wb[, c(af, "Vendace_recruits_50")]
cc1
...
sp <- ggplot(logcc2.1, aes(x = Vendace_recruits_50, y = value)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth() +
  facet_wrap(~ abiotic_factor, scales = "free_y") +
  labs(title = "Abiotic factors vs Vendace recruits",
       x = "Vendace_recruits_50",
...
library(reshape2)
vencm <- cm["Vendace_recruits_50", af]
vencm2 <- matrix(vencm, nrow = 1)
rownames(vencm2) <- "Vendace_recruits_50"
colnames(vencm2) <- names(vencm)
...
str(logcc6)
multi_hmm <- depmix(list(SST_spr ~ 1, SHT_su ~ 1, SST_su ~ 1, SAL_win ~ 1, SAL_deep ~ 1, pH ~ 1, 
                         Ox_deep ~ 1, DIP ~ 1, TP ~ 1, Si ~ 1, TN ~ 1, DIN ~ 1, Ice_max ~ 1, 
                         N_load ~ 1, P_load ~ 1, WBSI ~ 1, Vendace_recruits_50 ~ 1),
...
emission_prob <- getpars(multi_hmm_fit)[7:74]
emission_prob2 <- matrix(emission_prob, nrow = 2, byrow = TRUE)
rownames(emission_prob2) <- c("R1", "R2")
vars <- c("SST_spr", "SHT_su", "SST_su", "SAL_win", "SAL_deep", "pH", "Ox_deep", "DIP", "TP", "Si", "TN", 
          "DIN", "Ice_max", "N_load", "P_load", "WBSI", "Vendace_recruits_50")
...
dbn_data <- logcc6.1[, c("State", "SHT_su", "Si", "Vendace_recruits_50")]
dbn_data
dbn_data2 <- dbn_data %>%
  mutate(across(c(State, SHT_su, Si, Vendace_recruits_50), 
                ~lag(.), .names = "{.col}_t")) %>%
  rename_with(~paste0(.x, "_t1"), c(State, SHT_su, Si, Vendace_recruits_50)) %>%
  filter(!is.na(State_t))
dbn_data2

model_string <- "[State_t][State_t1|State_t][SHT_su_t|State_t][Si_t|State_t][Vendace_recruits_50_t|State_t:SHT_su_t:Si_t][SHT_su_t1|SHT_su_t:State_t1][Si_t1|Si_t:State_t1][Vendace_recruits_50_t1|Vendace_recruits_50_t:Si_t1:SHT_su_t1]"
dbn_str <- model2network(model_string)

fitted_dbn <- bn.fit(dbn_str, data = dbn_data2)

dbn_plot <- graphviz.plot(dbn_str, main = "2-Slice DBN (Vendace Recruitment)")
fitted_dbn
...
for (i in 1:n_years) {
  ...
  predicted_values[i] <- predict(fitted_data, node = "Vendace_recruits_50_t1", data = current_input)
}

actual_values <- dbn_data2$Vendace_recruits_50_t1
plot_df <- data.frame(
  Year = rep(years, 2),
...
ggplot(plot_df, aes(x = Year, y = Recruitment, color = Type)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Vendace Recruitment: Actual vs Predicted",
       x = "Year",
       y = "Vendace Recruitment") +
