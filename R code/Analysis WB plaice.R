...
cc1 <- wb[, c(af, "plaice_recruitment")]
cc1
...
sp <- ggplot(logcc2.1, aes(x = plaice_recruitment, y = value)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth() +
  facet_wrap(~ abiotic_factor, scales = "free_y") +
  labs(title = "Abiotic factors vs Plaice recruits",
       x = "Plaice_recruitment",
...
print(cm)
cm["plaice_recruitment", af]
cor.test(cc2$NH4, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$NO3, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$NO2, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$TP, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$Si, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$TN, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$DIP, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$DIN, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$SST_win, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$SST_spr, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$SST_su, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$SAL_win, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$Ox_deep, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$N_load, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$P_load, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$Ice_max, cc2$plaice_recruitment, method = "spearman")
cor.test(cc2$WBSI, cc2$plaice_recruitment, method = "spearman")

install.packages("corrplot")
library(corrplot)
plaicecm <- cm["plaice_recruitment", af]
plaicecm2 <- matrix(plaicecm, nrow = 1)
rownames(plaicecm2) <- "plaice_recruitment"
colnames(plaicecm2) <- names(plaicecm)
corrplot(plaicecm2,
         method = "color",
         is.corr = TRUE,
         col.lim = c(-1, 1),
         ...
         tl.col = "black",
         number.cex = 0.8,
         title = "Correlation with Plaice Recruits",
         mar = c(3, 0, 3, 0))
...
multi_hmm <- depmix(list(NH4 ~ 1, NO3 ~ 1, NO2 ~ 1, TP ~ 1, Si ~ 1, TN ~ 1, DIP ~ 1, DIN ~ 1, SST_win ~ 1, 
                         ...
                         Ice_max ~ 1, WBSI ~ 1, plaice_recruitment ~ 1),
...
vars <- c("NH4", "NO3", "NO2", "TP", "Si", "TN", "DIP", "DIN", 
          ...
          "N_load", "P_load", "Ice_max", "WBSI", "plaice_recruitment")
...
dbn_data <- logcc6.1[, c("State", "NO3", "Si", "plaice_recruitment")]
dbn_data
dbn_data2 <- dbn_data %>%
  mutate(across(c(State, NO3, Si, plaice_recruitment), 
                ~lag(.), .names = "{.col}_t")) %>%
  rename_with(~paste0(.x, "_t1"), c(State, NO3, Si, plaice_recruitment)) %>%
  filter(!is.na(State_t))
dbn_data2

model_string <- "[State_t][State_t1|State_t][NO3_t|State_t][Si_t|State_t][plaice_recruitment_t|State_t:NO3_t:Si_t][NO3_t1|NO3_t:State_t1][Si_t1|Si_t:State_t1][plaice_recruitment_t1|plaice_recruitment_t:Si_t1:NO3_t1]"
dbn_str <- model2network(model_string)

fitted_dbn <- bn.fit(dbn_str, data = dbn_data2)

dbn_plot <- graphviz.plot(dbn_str, main = "2-Slice DBN (Plaice Recruitment)")
fitted_dbn
...
for (i in 1:n_years) {
  ...
  predicted_values[i] <- predict(fitted_data, node = "plaice_recruitment_t1", data = current_input)
}

actual_values <- dbn_data2$plaice_recruitment_t1
plot_df <- data.frame(
  Year = rep(years, 2),
...
ggplot(plot_df, aes(x = Year, y = Recruitment, color = Type)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Plaice Recruitment: Actual vs Predicted",
       x = "Year",
       y = "Plaice Recruitment") +
